...
class Lane:
  def __init__(self, orig_frame):
    ...
    self.roi_points = np.float32([
      (289, 210), # Top-left corner
      (30, 400),   # Bottom-left corner
      (609, 400), # Bottom-right corner
      (330, 210)  # Top-right corner
    ])
    # width,640 ,heigth,426
  ...
  def perspective_transform(self, frame=None, plot=False):
    ...
    print('size',self.warped_frame.shape) # 全部0になっている
...
def lane_tracker_image(image_path):
    ...
    # Overlay lines on the original frame
    frame_with_lane_lines, save_img = lane_obj.overlay_lane_lines()
    ...
    # Display curvature and center offset on image
    frame_with_lane_lines2 = lane_obj.display_curvature_offset(frame=frame_with_lane_lines, plot=True)
    cv2.imshow('Processed Image', frame_with_lane_lines2)
    cv2.waitKey(0)
    cv2.destroyAllWindows()


def lane_tracker_image2(image_path):
    original_frame = cv2.imread(image_path)
    lane_obj = Lane(orig_frame=original_frame)
    ...
    cv2.waitKey(0)
    print(lane_obj.width,lane_obj.height)
    ...
    # 4. スライディングウィンドウによるレーンライン検出
    left_fit, right_fit = lane_obj.get_lane_line_indices_sliding_windows()
    frame_with_lane_lines, _ = lane_obj.overlay_lane_lines()
    cv2.imshow('Detected Lane Lines', frame_with_lane_lines)
    cv2.waitKey(0)
    ...
    # 6. カーブ半径とオフセットを表示
    frame_with_lane_lines2 = lane_obj.display_curvature_offset(frame=frame_with_lane_lines, plot=True)
    cv2.imshow('Curvature and Offset', frame_with_lane_lines2)
    cv2.waitKey(0)
...
if __name__ == '__main__':
    import os
    image_path = 'test_img/image2.png'
    lane_tracker_image2(image_path)
